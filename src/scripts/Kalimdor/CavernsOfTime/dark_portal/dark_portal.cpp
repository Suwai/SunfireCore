...
{
    {-1954.875, 7199.664, 22.361},
    {-1884.577, 7105.649, 22.641},
    {-1953.659, 7013.159, 22.436},
    {-2036.578, 7024.884, 23.153},
};
...
struct mob_medivhAI : public ScriptedAI
{
    ...
    void JustDied(Unit* Killer)
    {
        ...
        //m_creature->GetMap()->UpdateWorldState(2541, 0);
            ...
            switch(pInstance->GetData(DATA_WAVE))
            {
                ...
                case 0:
                    if(Wave_Timer < diff)
                    {
                        ...
                        Wave_Timer = 9000;
                        //m_creature->GetMap()->UpdateWorldState(2784, pInstance->GetData(DATA_WAVE));
...
struct mob_time_riftAI : public ScriptedAI
{
    ...
    void SummonRandomGuardian()
    {
        ...
        case 6:
            if(HeroicMode)
            {    
                if( pInstance->GetData(DATA_CHRONOLORDDEJADEATH) == 1) Guardian = DoSpawnCreature(H_MOB_CHRONO_LORD, rand()%5, rand()%5, 0, 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 60000);
                else Guardian = DoSpawnCreature(BOSS_CHRONO_LORD_DEJA, rand()%5, rand()%5, 0, 0, TEMPSUMMON_DEAD_DESPAWN, 600000);
            }
            else 
            {
                Guardian = DoSpawnCreature(BOSS_CHRONO_LORD_DEJA, rand()%5, rand()%5, 0, 0, TEMPSUMMON_DEAD_DESPAWN, 600000);
            }
            break;
        case 12:
            if(HeroicMode)
            {
                if( pInstance->GetData(DATA_TEMPORUSDEATH) == 1) Guardian = DoSpawnCreature(H_MOB_TIMEREAVER, rand()%5, rand()%5, 0, 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 60000);
                else Guardian = DoSpawnCreature(BOSS_TEMPORUS, rand()%5, rand()%5, 0, 0, TEMPSUMMON_DEAD_DESPAWN, 600000);
            }
            else
            {
                Guardian = DoSpawnCreature(BOSS_TEMPORUS, rand()%5, rand()%5, 0, 0, TEMPSUMMON_DEAD_DESPAWN, 600000);
            }
            break;
        case 18:
            Guardian = DoSpawnCreature(BOSS_AEONUS, rand()%5, rand()%5, 0, 0, TEMPSUMMON_DEAD_DESPAWN, 600000);
            break;
        default:
            switch(rand()%2)
            {
            case 0:
                //Rift Keeper
                Guardian = DoSpawnCreature(MOB_RIFT_KEEPER, rand()%5, rand()%5, 0, 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 60000);
                break;
            case 1:
                //Rift Lord
                Guardian = DoSpawnCreature(MOB_RIFT_LORD, rand()%5, rand()%5, 0, 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 60000);
                break;
            }
            break;
        }
...
struct mobs_infinite_adds : public ScriptedAI
{
    ...
    void Reset()
    {
        //m_creature->setActive(true); // cause Crash ... isnt neccesary
        IsChanneling = false;
        // Corrupt Medivh
        Corrupt_Timer = 3000;
        ...
        if(IsChanneling)
        {
            if(Corrupt_Timer < diff)
            {
                if(pInstance)
                {
                    ...
                    if(pMedivh)
                    {
                        ((mob_medivhAI *)pMedivh->AI())->UpdateWorldState(2540, ShieldPercent-1);
...
struct npc_saat : public ScriptedAI
{
...
struct mob_time_keeper : public ScriptedAI
{

...
struct boss_baron_rafe_dreugerAI : public boss_moroes_guestAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
		...
		if (HammerOfJustice_Timer <= diff)
        {
            if (Unit *pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 100, true))
            {
                DoCast(pTarget, SPELL_HAMMEROFJUSTICE);
...
struct boss_lady_catriona_von_indiAI : public boss_moroes_guestAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        {
            Unit *pTarget = DoSelectLowestHpFriendly(40.0f);

            DoCast(pTarget, SPELL_PWSHIELD);
        ...
        {
            if (Unit* pTarget = urand(0, 1) ? SelectTarget(SELECT_TARGET_RANDOM, 0, 30.0f, true) : DoSelectLowestHpFriendly(40.0f))
...
struct boss_lady_keira_berrybuckAI : public boss_moroes_guestAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        boss_moroes_guestAI::UpdateAI(diff);
        if ((HealthBelowPct(30)) && (DivineShield_Timer <=diff))
        {
        ...
        {
            Unit *pTarget = DoSelectLowestHpFriendly(100.0f);
...
struct boss_lord_crispin_ferenceAI : public boss_moroes_guestAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
		...
		if ((HealthBelowPct(30)) && (ShieldWall_Timer <=diff))
        {
			DoCast(me, SPELL_SHIELDWALL);
			ShieldWall_Timer = 120000;

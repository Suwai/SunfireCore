...
void CreatePet(Player *player, Creature * m_creature, uint32 entry) {
    ...
    if(player->GetPet())
        {
        m_creature->Say("Abandon your pet first", LANG_UNIVERSAL, 0);
        player->PlayerTalkClass->CloseGossip();
        return;
    }
...
bool GossipHello_npc_beastmaster(Player *player, Creature *_Creature)
{
    ...
    //player->ADD_GOSSIP_ITEM(0, GOSSIP_ITEM_STABLE, GOSSIP_SENDER_MAIN, GOSSIP_OPTION_STABLEPET);
...
//    player->ADD_GOSSIP_ITEM(0, GOSSIP_ITEM_UNTRAINEPET, GOSSIP_SENDER_MAIN, GOSSIP_OPTION_UNLEARNPETSKILLS);
...
bool GossipSelect_npc_beastmaster(Player *player, Creature *_Creature, uint32 sender, uint32 action)
{
    switch (action)
    {
        ...
        case GOSSIP_ACTION_INFO_DEF + 2: //Wolf
            CreatePet(player, _Creature, 21956);
            break;
        ...
        case GOSSIP_ACTION_INFO_DEF + 4: //SCARAB
            CreatePet(player, _Creature, 922);
            break;
